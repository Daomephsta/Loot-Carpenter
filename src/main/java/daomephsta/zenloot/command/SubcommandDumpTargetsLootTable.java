...
import daomephsta.zenloot.mixin.EntityLivingAccessors;
        ...
        if (sender instanceof Entity)
        {
            ...
            case BLOCK:
                ...
                if (te instanceof ILootContainer)
                {
                    ...
                    if (tableId == null && te instanceof ZenLootGeneratedFrom)
                        tableId = ((ZenLootGeneratedFrom) te).ZenLoot_getGeneratedFrom();
                }
                else
                    sender.sendMessage(ZenLoot.translation(".commands.dump.target.noTable"));
                break;
            case ENTITY:
                if (target.entityHit instanceof EntityLiving)
                {
                    tableId = ((EntityLivingAccessors) target.entityHit).callGetLootTable();
                    if (tableId == null && target.entityHit instanceof ZenLootGeneratedFrom)
                        tableId = ((ZenLootGeneratedFrom) target.entityHit).ZenLoot_getGeneratedFrom();
                }
                else
                    sender.sendMessage(ZenLoot.translation(".commands.dump.target.noTable"));
                break;
            case MISS:
                sender.sendMessage(ZenLoot.translation(".commands.dump.target.noTarget"));
                return;
            default:
                return;
            }
            ...
            sender.sendMessage(ZenLoot.translation(".commands.dump.target.senderNotEntity"));
    }
